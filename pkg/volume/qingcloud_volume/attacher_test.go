...
func (testcase *testcase) AttachVolume(volumeID string, instanceID string) (string, error) {
	...
	expectedInstanceID := qingcloud.NodeNameToInstanceID(expected.nodeName)
	if expectedInstanceID != instanceID {
		testcase.t.Errorf("Unexpected AttachVolume call: expected nodeName %s, got %s", expected.nodeName, instanceID)
		return "", errors.New("Unexpected AttachVolume call: wrong nodeName")
	}

	glog.V(4).Infof("AttachVolume call: %s, %s, returning %q, %v", volumeID, instanceID, expected.retDeviceName, expected.ret)

	return expected.retDeviceName, expected.ret
}

func (testcase *testcase) DetachVolume(volumeID string, instanceID string) error {
	...
	expectedInstanceID := qingcloud.NodeNameToInstanceID(expected.nodeName)
	if expectedInstanceID != instanceID {
		testcase.t.Errorf("Unexpected DetachVolume call: expected nodeName %s, got %s", expected.nodeName, instanceID)
		return errors.New("Unexpected DetachVolume call: wrong nodeName")
	}

	glog.V(4).Infof("DetachVolume call: %s, %s, returning %v", volumeID, instanceID, expected.ret)

	return expected.ret
}

func (testcase *testcase) VolumeIsAttached(volumeID string, instanceID string) (bool, error) {
	...
	expectedInstanceID := qingcloud.NodeNameToInstanceID(expected.nodeName)
	if expectedInstanceID != instanceID {
		testcase.t.Errorf("Unexpected VolumeIsAttached call: expected nodeName %s, got %s", expected.nodeName, instanceID)
		return false, errors.New("Unexpected VolumeIsAttached call: wrong nodeName")
	}

	glog.V(4).Infof("VolumeIsAttached call: %s, %s, returning %v, %v", volumeID, instanceID, expected.isAttached, expected.ret)

	return expected.isAttached, expected.ret
}

func (testcase *testcase) DisksAreAttached(volumeIDs []string, instanceID string) (map[string]bool, error){
	attached := make(map[string]bool, len(volumeIDs))
	return attached, nil
}
