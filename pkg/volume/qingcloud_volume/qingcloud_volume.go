...
func (plugin *qingcloudVolumePlugin) Init(host volume.VolumeHost) error {
	glog.V(4).Infof("qingcloudVolumePlugin Init(%v) called", host)
	plugin.host = host
	return nil
}
...
func (plugin *qingcloudVolumePlugin) NewMounter(spec *volume.Spec, pod *api.Pod, _ volume.VolumeOptions) (volume.Mounter, error) {
	glog.V(4).Infof("qingcloudVolumePlugin NewMounter(%+v, %+v) called", spec, pod)
	// Inject real implementations here, test through the internal function.
	return plugin.newMounterInternal(spec, pod.UID, &QingVolumeManager{}, plugin.host.GetMounter())
}
...
func (plugin *qingcloudVolumePlugin) NewUnmounter(volName string, podUID types.UID) (volume.Unmounter, error) {
	glog.V(4).Infof("qingcloudVolumePlugin NewUnmounter(%+v, %+v) called", volName, podUID)
	// Inject real implementations here, test through the internal function.
	return plugin.newUnmounterInternal(volName, podUID, &QingVolumeManager{}, plugin.host.GetMounter())
}
...
func (b *qingcloudVolumeMounter) CanMount() error {
	return nil
}
